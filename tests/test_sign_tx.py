...
ELEMENTS_ASSET_REISSUANCE_VECTORS = path.join(path.dirname(path.realpath(__file__)), "sign_tx_elements_reissue_test_vectors.json")
...
@pytest.fixture
def sign_tx_elements_reissue_test_vectors():
    with open(ELEMENTS_ASSET_REISSUANCE_VECTORS) as f:
        return json.load(f)
...
def test_sign_elements_asset_reissuance(sign_tx_elements_reissue_test_vectors, tmpdir):
    keys_dir = tmpdir.mkdir("ssm_keys")
    for k, v in sign_tx_elements_reissue_test_vectors.items():
        fingerprints, paths, values, prev_tx = prepare_signature(CHAINS[1], v.copy(), keys_dir)
        if prev_tx == v["signed_tx"][0]:
            raise ValueError("Provided unsigned and signed tx are identical")
        tx_out = sign_tx(CHAINS[1], prev_tx, fingerprints, paths, values, keys_dir)
